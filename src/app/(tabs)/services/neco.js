    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.hero}>
        <View style={styles.heroContent}>
          <View style={styles.badge}><Text style={styles.badgeText}>EDUCATION</Text></View>
          <Text style={styles.heroTitle}>NECO Token</Text>
          <Text style={styles.heroSubtitle}>Purchase NECO scratch cards instantly</Text>
        </View>
        <Image source={require('../../../../assets/education/neco-logo.png')} style={styles.heroImage} />
      </View>
      ...
      <View style={styles.quantityRow}>
        {[1, 2, 3, 4, 5].map((q) => (
          <Button key={q} title={String(q)} variant={quantity === q ? 'primary' : 'secondary'} onPress={() => setQuantity(q)} style={styles.qtyBtn} />
        ))}
      </View>

      <Input label="Phone Number (for PIN delivery)" value={phoneNumber} onChangeText={setPhoneNumber} placeholder="Enter phone number" keyboardType="phone-pad" icon="call-outline" required />
      <Input label="Email (Optional)" value={email} onChangeText={setEmail} placeholder="Enter email address" keyboardType="email-address" icon="mail-outline" />
      ...
      <Button title={loading ? 'Processing...' : 'Purchase Token'} onPress={handlePurchase} loading={loading} disabled={!phoneNumber || loading} style={styles.purchaseBtn} />
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  badgeText: { color: '#A5B4FC', fontSize: 10, fontWeight: '700', letterSpacing: 1 },
  ...
  summary: { backgroundColor: '#EEF2FF', borderLeftWidth: 4, borderLeftColor: '#4F46E5', borderRadius: 12, padding: 16, marginTop: 4, gap: 8 },
  ...
  summaryTotal: { borderTopWidth: 1, borderTopColor: '#C7D2FE', paddingTop: 10, marginTop: 4 },
  totalLabel: { fontSize: 16, color: '#4F46E5', fontWeight: '800' },
  totalValue: { fontSize: 18, color: '#4F46E5', fontWeight: '800' },
  purchaseBtn: { marginTop: 24, backgroundColor: '#4F46E5' },
});
